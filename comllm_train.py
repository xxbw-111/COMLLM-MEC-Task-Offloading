"""COMLLM training entry point with LACS future-impact reward shaping."""
...
try:
    from cost_calculator import calculate_cost, predict_multiturn_impact
...
# -sft-model-path	SFT 后的模型路径。COMLLM/GRPO 是在 SFT 模型基础上继续训练的，所以这里要填已经 SFT 好的模型目录。
# --dataset-path	GRPO/COMLLM 阶段使用的训练数据路径。
# --output-dir	训练输出目录，用来保存 checkpoint 和最终的 final_grpo_adapter。
# --max-seq-length	最大序列长度，也就是 prompt + completion 的最大 token 长度。超过这个长度的样本会被过滤掉。默认 2048。
# --epochs	训练轮数。默认 1。
# --learning-rate	学习率。默认 5e-6。
# --beta	GRPO 里的 KL 惩罚系数，用来限制模型不要偏离原始 SFT 模型太远。默认 0.005。
# --num-generations	GRPO 每个 prompt 采样多少个回答，也就是 group size。论文里通常对应 G。
# --generation-batch-size	生成回答时的 batch size。一般要和 num-generations 配合，显存不够时调小。
# --lacs-weight	LACS 未来影响项的权重。越大，模型越重视当前决策对未来队列拥塞的影响。默认 0.3。
# --trust-remote-code	是否允许 Hugging Face 加载模型仓库里的自定义代码。Qwen 这类模型有时需要加；如果不需要就不加。默认加。


def parse_args():
    parser = argparse.ArgumentParser(description="Run COMLLM GRPO training with LACS reward shaping.")
    parser.add_argument("--sft-model-path", required=True, help="Merged SFT model path used as the COMLLM base.")
    parser.add_argument("--dataset-path", required=True, help="GRPO/COMLLM JSON dataset path.")
    parser.add_argument("--output-dir", default="./outputs/comllm", help="Directory for COMLLM checkpoints.")
    ...
    parser.add_argument("--lacs-weight", type=float, default=0.3, help="Weight for the LACS future-impact term.")
...
def mec_reward_function(prompts: list, completions: list, **kwargs) -> list[float]:
    ...
    for i, completion_item in enumerate(completions):
        ...
        if model_decision is None:
            ...
            # --- Step A: 计算当前这一步的成本 (Current Cost) ---
            current_cost = calculate_cost(action_id, input_data)
            # --- Step B: 计算未来多轮影响 (Future Impact) ---
            future_cost = 0.0

            # 只有当决策有效(不是幻觉)，且不是本地执行时，才计算未来影响
            if current_cost < 10.0:
                try:
                    # 预测未来 2-3 步的影响
                    future_cost = predict_multiturn_impact(action_id, input_data, lookahead_steps=3)
                except Exception as e:
                    future_cost = 0.0  # 降级处理

            # --- Step C: 组合综合成本 (Total Cost) ---
            total_cost = current_cost + (W_FUTURE * future_cost)
            # 奖励是成本的负数
            task_reward += -total_cost

            if total_cost > 10.0:
                task_reward -= 10.0

        final_reward = (task_reward * TASK_REWARD_WEIGHT) + (format_reward * FORMAT_REWARD_WEIGHT)   #这里没有在前一项进行尺度上的归一
